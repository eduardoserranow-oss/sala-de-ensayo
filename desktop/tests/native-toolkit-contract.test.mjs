...
assert.deepEqual(
  contract.normalizeToolkitRequest({ stageId:'12:1788033523000:7', selection:'texture', projectName:'Mi canción' }),
  { stageId:'12:1788033523000:7', selection:'texture', projectName:'Mi canción' }
);
...
for (const token of [
  ...
  "path.resolve(root, projectName, 'FORTISSIMO MIDI')",
  'path.relative(root, projectDirectory)',
  'projectMidiDirectory(rootDirectory, request.projectName)',
...
]) assert.ok(main.includes(token), `Phase 8 native toolkit main-process contract missing: ${token}`);
...
for (const token of [
  ...
  "saveStagedMidi: (stageId, selection = 'pair', projectName = 'Untitled Direction')",
  ...
  'desktop-workspace8'
]) assert.ok(preload.includes(token), `Phase 8 preload toolkit bridge missing: ${token}`);
...
]) assert.ok(ui.includes(token), `Phase 6 fallback toolkit UX missing: ${token}`);
...
for (const token of [
  ...
  "api.saveStagedMidi(staged.stageId,selection,title)",
  'midi-project-workflow',
  'Working title / FORTISSIMO MIDI'
...
console.log('PASS FORTISSIMO Desktop Phase 8 project workflow: safe project names, automatic Working title/FORTISSIMO MIDI folders, selective MIDI drag/save, and narrow trusted IPC.');
