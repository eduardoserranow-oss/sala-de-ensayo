...
for (const token of [
  "const BRIDGE_VERSION = '13.1.0'","'midi-drag-selective'","'midi-export-folder'","'midi-export-native'","'midi-project-workflow'","'project-session-intelligence'","'full-session-recall'","'project-version-history'","'stable-desktop-release'",'chooseMidiExportFolder: () => ipcRenderer.invoke(MIDI_EXPORT_FOLDER_CHANNEL)',"saveStagedMidi: (stageId, selection = 'pair', projectName = 'Untitled Direction')",'normalizeToolkitRequest({ stageId, selection: normalizeMidiSelection(selection), projectName })','openMidiExportFolder: () => ipcRenderer.invoke(MIDI_EXPORT_OPEN_CHANNEL)','desktop-workspace13','project-versions13'
]) assert.ok(preload.includes(token), `Phase 12 preload toolkit bridge missing: ${token}`);

for (const token of ['ARRASTRAR MIDI','ARRASTRAR MIDI HACIA DAW','DESKTOP · NATIVE FILE DRAG','Escritorio',"api.startMidiDrag(stagedMidi.stageId,selection)",'MutationObserver']) assert.ok(ui.replaceAll('↗ ', '').includes(token), `Phase 14.1b native drag UX missing: ${token}`);
for (const removedToken of ['ARRASTRAR MAIN','ARRASTRAR TEXTURES','vr-midi-drag14b-layers']) assert.ok(!ui.includes(removedToken), `Redundant split drag control still present: ${removedToken}`);
