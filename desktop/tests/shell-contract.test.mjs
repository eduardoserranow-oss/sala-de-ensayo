...
assert.ok(!main.includes('startDrag('), 'Phase 1 must not implement native MIDI drag yet.');
assert.ok(!main.includes('ipcMain'), 'Phase 1 must not expose privileged IPC handlers yet.');
assert.ok(!main.includes("require('node:fs')"), 'Phase 1 shell must not access the filesystem.');
assert.ok(!main.includes("require('node:child_process')"), 'Phase 1 shell must not spawn child processes.');
...
assert.ok(!preload.includes('ipcRenderer'), 'Phase 1 preload must not expose or use renderer IPC.');
assert.ok(!preload.includes('invoke('), 'Phase 1 renderer must not have privileged request IPC.');
...
console.log('PASS FORTISSIMO Desktop Phase 1 shell contract: remote web source of truth, isolated renderer, no native MIDI/filesystem privileges yet.');
