...
for (const token of [
  ...
  'did-finish-load',
  'mainWindow.show()'
]) assert.ok(splashWindow.includes(token), `Native splash contract missing: ${token}`);

for (const token of [
  'Developed, programmed &amp; calibrated by SERRA.',
  'Built for the creative development of the SERRA artist team.',
  'Santo Domingo · Dominican Republic · 2026',
  'Desktop v${version}',
  'Loading FORTISSIMO workspace…'
]) assert.ok(splashHtml.includes(token), `Splash content missing: ${token}`);

for (const token of [
  'fortissimo-header-logo-v6.jpg',
  'fortissimo-icon-20260824.svg',
  'extraResource: [dragIcon, splashLogo, splashMark]'
]) assert.ok(forge.includes(token), `Splash packaging asset missing: ${token}`);

for (const token of [
  'WIDE_LAYOUT_STYLESHEET_URL',
  'installAdaptiveWorkspace()',
...
for (const token of [
  ...
  '.vr-shell{width:min(1680px,100%)',
  '.vr-result-layout{display:grid',
  '@media (min-width:1500px)',
  '.eightbar-wrap{grid-template-columns:repeat(2,minmax(0,1fr))',
  '@media (max-width:1179px)'
]) assert.ok(wideCss.includes(token), `Adaptive wide-screen CSS missing: ${token}`);
