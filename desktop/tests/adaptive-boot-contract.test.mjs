...
for (const token of ['const MIN_SPLASH_VISIBLE_MS = 8000','version: app.getVersion()',"logo: splashAssetUrl('fortissimo-header-logo-v6.jpg')", "mark: splashAssetUrl('forte-flex-favicon.svg')",'holdMs: String(MIN_SPLASH_VISIBLE_MS)','nodeIntegration: false','contextIsolation: true','sandbox: true','webSecurity: true','did-finish-load','mainWindow.show()']) assert.ok(splashWindow.includes(token), `Native splash contract missing: ${token}`);
for (const token of ['Developed, programmed &amp; calibrated by SERRA.','Built for the creative development of the SERRA artist team.','Santo Domingo · Dominican Republic · 2026','Desktop v${version}','Preparing FORTISSIMO workspace…','class="progress"','@keyframes bootProgress']) assert.ok(splashHtml.includes(token), `Splash content missing: ${token}`);
for (const token of ['fortissimo-header-logo-v6.jpg','forte-flex-favicon.svg','fortissimo-desktop-icon.png','extraResource: [dragIcon, desktopIconPng, splashLogo, splashMark]']) assert.ok(forge.includes(token), `Splash/icon packaging asset missing: ${token}`);
...
console.log('PASS FORTISSIMO Desktop Phase 14.1b keeps adaptive workspace, eight-second native boot, original icon and background audio intact.');
