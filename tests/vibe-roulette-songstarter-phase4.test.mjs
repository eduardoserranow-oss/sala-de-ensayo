...
const starter=buildSongStarterProducerPlan(arrangement,{
  ...
  bpm:104,energyTarget:0.76,mood:'connection',emotionFilters:['hope','intimacy'],seed:'phase42-ci'
});
...
assert.equal(starter.phase,4.2);
assert.ok(starter.activeLayerCount>=1&&starter.activeLayerCount<=2);
assert.deepEqual(starter.layers.map(layer=>layer.role),starter.support?['foundation','support']:['foundation']);
assert.equal(starter.layers[0].preset,'About Time');
if(starter.support)assert.ok(['Always Danger','Broad Texture'].includes(starter.layers[1].preset));
...
if(starter.support){
  ...
  assert.ok(support.events.length>0,'Support must compose real independent MIDI events');
  assert.ok(support.events.every(event=>event.layerRole==='support'));
  assert.ok(support.events.some(event=>event.pass==='A′'&&event.startBeat>=16),'Support must cover A-prime before Phase 5 arrangement entry/exit logic');
  assert.ok(support.events.every(event=>event.startBeat>=0&&event.startBeat<32),'Support MIDI must stay inside the shared 8-bar clock');
  ...
  for(const event of support.events){
    ...
    assert.ok(safe.has(((event.midi%12)+12)%12),`support note ${event.midi} must stay harmonically safe for ${allChords[event.chordIndex]}`);
  }
...
assert.ok(starter.exportFiles.length>=1&&starter.exportFiles.length<=2);
assert.equal(starter.exportFiles[0],'01_Foundation_About-Time.mid');
if(starter.support)assert.ok(starter.exportFiles[1].startsWith('02_Support_')&&starter.exportFiles[1].endsWith('.mid'));
assert.equal(starter.exportFiles.some(name=>name.includes('03_Hook_')),false,'Phase 4.2 may never export an active Hook MIDI');
...
]) assert.ok(runtime.includes(token),`missing Song Starter runtime token: ${token}`);
...
console.log('PASS Song Starter Phase 4.2: Foundation + Support/Texture active lock, no Hook playback/export, Foundation invariance, shared 8-bar clock and drums/fallback protection');
