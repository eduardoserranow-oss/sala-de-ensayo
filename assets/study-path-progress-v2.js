...
function normalizeSong(s){return`${String(s?.artist||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g," ").trim()}::${String(s?.title||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g," ").trim()}`}
function sameSong(a,b){const ka=normalizeSong(a),kb=normalizeSong(b);return ka!=="::"&&ka===kb}
function reconcileAfterAdapt(projectId,oldStages=[],newStages=[]){const all=readAll(),oldProject=all[projectId]||{stages:{},updatedAt:0},songProgress=new Map();(oldStages||[]).forEach((st,i)=>{const sid=String(st?.id||`stage-${i+1}`),ps=oldProject.stages?.[sid]||freshStage();["a","b"].forEach(role=>{const key=normalizeSong(st?.[role]);if(key!=="::"&&!songProgress.has(key))songProgress.set(key,JSON.parse(JSON.stringify(ps?.[role]||freshSong())))});});const next={stages:{},updatedAt:Date.now()};(newStages||[]).forEach((st,i)=>{const sid=String(st?.id||oldStages?.[i]?.id||`stage-${i+1}`),oldStage=oldStages?.[i],oldSid=String(oldStage?.id||sid),oldState=oldProject.stages?.[oldSid]||freshStage(),a=songProgress.get(normalizeSong(st?.a))||freshSong(),b=songProgress.get(normalizeSong(st?.b))||freshSong(),sameA=sameSong(oldStage?.a,st?.a),sameB=sameSong(oldStage?.b,st?.b);next.stages[sid]={a:JSON.parse(JSON.stringify(a)),b:JSON.parse(JSON.stringify(b)),completed:!!oldState.completed&&sameA&&sameB};});all[projectId]=next;writeAll(all);window.dispatchEvent(new CustomEvent("fortissimo:path-progress-reconciled",{detail:{projectId}}));return next}
...
window.FortissimoPathProgress={DEFAULT_CHECKS,get,stageState,currentIndex,isUnlocked,stats,toggleCheck,toggleLearned,completeStage,reconcileAfterAdapt,setActive,active};
