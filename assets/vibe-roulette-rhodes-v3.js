...
function dynamicsForEnergy(energy){
  return {
    bass:lerp(43,56,energy),
    right:lerp(48,65,energy),
    response:lerp(36,50,energy),
    topBoost:lerp(3.5,6,energy),
    fingerSpreadMs:lerp(29,17,energy),
    mainLength:lerp(3.45,2.55,energy),
    bassLength:lerp(3.75,3.05,energy)
  };
}
...
export function buildCommercialAfroRhodesPlan(chords,{
  roman=[],bars=4,beatsPerBar=4,bpm=96,energyTarget=0.65,mood='connection'
}={}){
  ...
  harmonicPlan.forEach((item,chordIndex)=>{
    ...
    voicing.left.forEach((midi,i)=>pushEvent(events,{
      midi,
      velocity:(dyn.bass+jitter(`bass-${chordIndex}-${i}`,2))*barArc,
      startBeat:item.startBeat,
      durationBeats:Math.min(span*0.94,dyn.bassLength*Math.max(0.55,span/4)),
    ...
    voicing.right.forEach((midi,i)=>{
      ...
      pushEvent(events,{
        midi,
        velocity:(dyn.right+(isTop?dyn.topBoost:i===0?1:-3.5)+jitter(`main-${chordIndex}-${i}`,2.2))*barArc,
        startBeat:item.startBeat+Math.min(0.13,0.035+energy*0.07),
        durationBeats:Math.min(span*0.87,dyn.mainLength*Math.max(0.50,span/4)),
    ...
    if(span>=4){
      const offsets=phraseOffsetsForBar(barIndex,energy);
      offsets.forEach((offset,responseIndex)=>{
        ...
        responseNotes.forEach((midi,i)=>pushEvent(events,{
          midi,
          velocity:dyn.response+(i===responseNotes.length-1?3:-2)+jitter(`resp-${chordIndex}-${responseIndex}-${i}`,1.8),
          startBeat:item.startBeat+offset,
          durationBeats:Math.min(0.58+(1-energy)*0.30,span-offset-0.08),
          role:'afro-tropical-response',chordIndex,
          fingerOffsetSeconds:i*0.010
        }));
      });
    }
  });
  ...
  return {
    ...
    style:'Afro-Tropical · Indie · Lo-Fi · Soulful · Commercial',
    profile:'commercial-afro-v1',
    bpm:clamp(Number(bpm)||96,40,220),
    bars,beatsPerBar,totalBeats:bars*beatsPerBar,energy,mood,
...
export function rotaryProfileForEnergy(value){
  ...
  return {
    drumHz:lerp(0.42,1.48,Math.pow(energy,1.25)),
    hornHz:lerp(0.68,2.72,Math.pow(energy,1.2)),
    wet:lerp(0.17,0.28,energy),
    depth:lerp(0.19,0.33,energy),
...
class RhodesSampleBank{
  ...
  async decode(ctx,layer,midi){
    ...
    this.bufferCache.set(key,promise); return promise;
  }
  async preload(performance){
    const unique=new Map();
    for(const event of performance.events){ const layer=velocityLayerForMidiVelocity(event.velocity); unique.set(this.key(layer,event.midi),[layer,event.midi]); }
...
export class CommercialAfroRhodesPreview{
  ...
  createChain(ctx,energy){
    ...
    const drive=ctx.createWaveShaper(); drive.curve=softDriveCurve(0.07+energy*0.04); drive.oversample='2x';
    const highShelf=ctx.createBiquadFilter(); highShelf.type='highshelf'; highShelf.frequency.value=2800; highShelf.gain.value=-2.8;
    const lowpass=ctx.createBiquadFilter(); lowpass.type='lowpass'; lowpass.frequency.value=lerp(4300,5100,energy); lowpass.Q.value=0.16;
    input.connect(drive); drive.connect(highShelf); highShelf.connect(lowpass);
    const dry=ctx.createGain(); dry.gain.value=1-rotary.wet*0.42;
    ...
    const lowDelay=ctx.createDelay(0.015),highDelay=ctx.createDelay(0.015); lowDelay.delayTime.value=0.0027; highDelay.delayTime.value=0.0019;
    ...
    const sum=ctx.createGain(); dry.connect(sum); lowPan.connect(sum); highPan.connect(sum);
    const comp=ctx.createDynamicsCompressor(); comp.threshold.value=-7; comp.knee.value=9; comp.ratio.value=1.18; comp.attack.value=0.028; comp.release.value=0.30; sum.connect(comp);
    ...
    attachLfo(rotary.drumHz,lowPan,lowDelay,0.00040); attachLfo(rotary.hornHz,highPan,highDelay,0.00068);
  ...
  async playFourBars(chords,options={}){
    const performance=buildCommercialAfroRhodesPlan(chords,options);
    const ctx=await this.ensureContext(); this.stop(); const chain=this.createChain(ctx,performance.energy); const secondsPerBeat=60/performance.bpm;
    const unique=new Map();
    for(const event of performance.events){ const layer=velocityLayerForMidiVelocity(event.velocity); unique.set(`${layer}:${event.midi}`,[layer,event.midi]); }
    ...
    for(const event of performance.events){
      const layer=velocityLayerForMidiVelocity(event.velocity); const buffer=decoded.get(`${layer}:${event.midi}`); if(!buffer) continue;
      ...
      const duration=Math.max(0.12,Math.min(buffer.duration-0.03,event.durationBeats*secondsPerBeat)); const end=start+duration;
      gain.gain.setValueAtTime(0.0001,start); gain.gain.exponentialRampToValueAtTime(roleGain,start+0.018); gain.gain.setValueAtTime(roleGain*0.86,Math.max(start+0.05,end-Math.min(0.22,duration*0.22))); gain.gain.exponentialRampToValueAtTime(0.0001,end);
      source.connect(gain); gain.connect(chain.input); source.start(start); source.stop(end+0.03); this.activeSources.add(source);
