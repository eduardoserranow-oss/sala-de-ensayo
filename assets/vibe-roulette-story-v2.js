...
export function analyzeStoryLocally(text,{title=''}={}){
  ...
  const profile={version:2,source:'fortissimo-story-intelligence-v2-contextual',text:String(text||'').trim(),title:String(title||'').trim(),primaryTerritory,secondaryTerritory,confidence,territoryScores,vibeSignals,energySuggestion:energy,tempoSuggestion:tempo,harmonicIntent:'',tags,analyzedAt:new Date().toISOString()};
...
function restoreProfile(){if(typeof window==='undefined')return null;try{const raw=localStorage.getItem(STORY_STORAGE_KEY);const stored=raw?JSON.parse(raw):null;if(stored?.version===2){const {emotionalState:discarded,...p}=stored;p.tags=(p.tags||[]).filter(tag=>!['#Amor','#Desamor','#Despecho','#Asfixia'].includes(tag));window.__FORTISSIMO_VIBE_STORY_PROFILE__=p;return p;}}catch(_){}return null;}
...
function installUi(){
  ...
  const titleField=document.getElementById('workingTitle')?.closest('.session-field');if(!titleField)return;const wrap=document.createElement('div');wrap.className='story-intel';wrap.innerHTML=`<label for="storyCreativeBrief">Story / Chapter / Creative Brief</label><textarea id="storyCreativeBrief" maxlength="2400" placeholder="Paste the story your team is writing from. Vibe Roulette will infer territory, vibe signals, Body Energy and tempo."></textarea><div class="story-intel-actions"><button class="story-analyze" id="storyAnalyzeBtn" type="button">Analyze story</button><span style="font-size:11px;color:rgba(255,255,255,.46)">Suggestion only — you can override Territory and Body Energy.</span></div><div class="story-analysis" id="storyAnalysisPanel"></div>`;titleField.insertAdjacentElement('afterend',wrap);
