(function(){
  ...
  function normalize(){
    ...
    if(ref){ref.classList.add("feature","feature-referencefinder","home-standard-module");ensureSimpleAction(ref,"Reference Finder","Buscar referencias","reference-finder.html?v=rf-preview1");ensureCue(ref);}
  }
  ...
  function bindUsage(){
    ...
    if(spin&&spin.dataset.homeUsageBound!=="1"){spin.dataset.homeUsageBound="1";spin.addEventListener("click",()=>{if(wheelCounted)return;wheelCounted=true;record("wheel",45);});}
  }
  ...
  function watch(){
    ...
    stackObserver=new MutationObserver(()=>{
      const n=modules.size;collect();normalize();moveWheel();if(modules.size!==n)render(false);mountPins();bindUsage();observeReveals();requestParallax();
    });
  ...
  function installStyles(){
    ...
    s.textContent=`
      ...
      .study-paths-home-final>.media{position:absolute;inset:0;z-index:0;background-size:cover!important;background-repeat:no-repeat!important;background-position:center!important;filter:brightness(.64) saturate(.95) contrast(1.04)}
      ...
      .feature-guitar>.media,.feature-bass>.media{background-size:cover!important;background-repeat:no-repeat!important;will-change:transform!important}
      .home-module-pin{position:absolute;z-index:30;top:max(110px,calc(env(safe-area-inset-top) + 88px));right:max(20px,env(safe-area-inset-right));display:grid;place-items:center;width:30px;height:30px;padding:6px;border:1px solid rgba(255,255,255,.09);border-radius:8px;background:rgba(5,5,5,.36);color:rgba(255,255,255,.82);cursor:pointer;filter:drop-shadow(0 2px 8px rgba(0,0,0,.62));backdrop-filter:blur(7px);pointer-events:auto!important}
      .home-module-pin[hidden]{display:grid!important}
      .home-module-pin svg{display:block;width:15px;height:15px;fill:currentColor;transform:rotate(38deg)}
      .home-module-pin[aria-pressed="true"]{color:#ff6500;border-color:rgba(255,101,0,.48);background:rgba(18,8,2,.58)}
      .home-module-pin[aria-pressed="true"] svg{transform:rotate(0)}
      .home-module-pin.home-pin-limit:not([aria-pressed="true"]){opacity:.48}
      ...
      @media(max-width:760px){
        .home-module-pin{top:max(104px,calc(env(safe-area-inset-top) + 78px));right:max(16px,env(safe-area-inset-right));width:30px;height:30px}
        .study-paths-home-final>.media{background-position:58% center!important;filter:brightness(.56) saturate(.92) contrast(1.05)}
        ...
        .study-paths-home-final .routine-content{max-width:min(520px,100%)!important}
        .study-paths-home-final .practice-btn{width:min(100%,302px)}
      }
    `;
